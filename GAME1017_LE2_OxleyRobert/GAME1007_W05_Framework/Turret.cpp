...
void Turret::Update()
{
	...
	m_angle = 0.0; // Reset angle every frame
	m_hasTarget = false;
	...
	for (unsigned i = 1; i < GameState::Enemies().size(); i++)
	{
		...
		if (MAMA::Distance(&turretCenter, &nextEnemy) < MAMA::Distance(&turretCenter, &closestEnemy))
		{
	...
	{
		SDL_FPoint deltaVector = { closestEnemy.x - turretCenter.x, closestEnemy.y - turretCenter.y };
		m_angle = MAMA::Rad2Deg(MAMA::AngleBetweenPoints(deltaVector.y, deltaVector.x)) + 90.0;
		m_hasTarget = true;
		if (m_fireCtr == 0) // Ready to fire! 
		{
			SDL_FPoint normalVector = MAMA::Normalize(deltaVector);
			GameState::Bullets().push_back(new Bullet({turretCenter.x - 2.0f, turretCenter.y - 2.0f, 4.0f, 4.0f}, normalVector.x, normalVector.y));
			m_fireCtr = s_coolDown; // Turret now on cooldown!
		}
